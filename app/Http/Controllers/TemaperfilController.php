...
public function autocomplete(Request $request)
    ...
    public function index(Request $request)
    {
            ...
            ->select('tema_perfil.*','nombresapellidos','nombre')
            ->get();
    ...
    public function edit($id)
    {
    	$tema_perfil=Tema_perfil::findOrFail($id);
         $docentes=DB::table('docente')->where('condicion','=','1')->get();
    	$postulantes = DB::table('postulante')->where('estado','=','Activo')->get();
        return view("uno.tema_perfil.edit",["tema_perfil"=>$tema_perfil,"docentes"=>$docentes,"postulantes"=>$postulantes]);
    ...
    public function update(TemaperfilFormRequest $request,$id)
    {
    	...
    	if($request->get('modalidad') == 'TRABAJO DIRIGIDO1'){
            $tema_perfil->modalidad="TRABAJO DIRIGIDO";
        }else if($request->get('modalidad') == 'PROYECTO DE GRADO1'){
            $tema_perfil->modalidad="PROYECTO DE GRADO";
        }else if($request->get('modalidad') == 'TESIS DE GRADO1'){
            $tema_perfil->modalidad="TESIS DE GRADO";
        }else{
            $tema_perfil->modalidad=$request->get('modalidad');
        }
        //$tema_perfil->modalidad=$request->get('modalidad');
        if ($request->get('modalidad') == "PROYECTO DE GRADO" || $request->get('modalidad')=="TESIS DE GRADO" || $request->get('modalidad')=="PROYECTO DE GRADO1" || $request->get('modalidad')=="TESIS DE GRADO1") {
           $tema_perfil->institucion="NULL";
            ...
            $tema_perfil->institucion=$request->get('institucionn');
...
public function reporte_generalperfil(){
      ...
      $vistareporte="uno.tema_perfil.reporteacta_perfil_general";
        ...
        ->join('postulante as p','te.idpostulante','=','p.idpostulante')
         ->join('docente as d','te.iddocente','=','d.iddocente')
        ->select('te.idtema_perfil','te.titulo_tema','te.modalidad','te.institucion','te.observacion','p.nombresapellidos as postulante','d.nombre','te.fecha_aprobacion','te.estados')
       ->orderBy('te.titulo_tema','asc')
        ->get();
